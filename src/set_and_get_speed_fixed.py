...
def get_speed():
    #print("get_speed() :: Start")
    global ticks
    utime.sleep(0.1)  # Wait for 1s
    #print("get_speed() :: get ticks")
    last_ticks = ticks
    #print("get_speed() :: putting to state machine")
    # Check if there's space in the TX FIFO before putting data
    if pulse_counter_sm.tx_fifo() < 4:  # 4 is the typical TX FIFO size
        pulse_counter_sm.put(0xFFFFFFFF)  # Trigger the PIO to push data and reset the counter
    else:
        print("TX FIFO is full. Skipping put() to avoid blocking.")
        # Optionally, you can choose to clear the FIFO or handle it differently
        # For example, you could skip this measurement or reset the FIFO    print("get_speed() :: get tick us")
